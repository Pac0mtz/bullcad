...
export function buildPlanSvg(model, opts = {}) {
  ...
  openings.forEach((o) => {
    ...
    if (o.type === 'door') {
      ...
      if (style === 'double') {
        ...
        // thin door slab parked in the wall pocket (white panel) + dashed ghost
        // across the opening (closed position)
        const pdir = o.hinge === 'right' ? -1 : 1;
        const slabT = th * 0.42;
        const px = pdir > 0 ? hw : -hw - o.width;
        inner += `<rect x="${r2(px)}" y="${r2(-slabT / 2)}" width="${r2(o.width)}" height="${r2(slabT)}" rx="0.05" fill="#ffffff" stroke="${NAVY}" stroke-width="0.05"/>`;
        inner += `<rect x="${r2(-hw)}" y="${r2(-slabT / 2)}" width="${r2(o.width)}" height="${r2(slabT)}" rx="0.05" fill="none" stroke="${NAVY}" stroke-width="0.04" stroke-dasharray="0.22 0.16"/>`;
  ...
  const roomLabelPos = model.roomLabelPos || {};
  detectRooms(walls).forEach((rm) => {
    const sig = roomSignature(roomWalls(rm, walls));
    const name = roomNames[sig] || '';
    const showArea = opts.showRoomAreas !== false;
    if (!name && !showArea) return;
    const lp = roomLabelPos[sig]; // honor a label the user dragged in-app
    const cx = lp ? lp.x : rm.centroid.x, cy = lp ? lp.y : rm.centroid.y;
